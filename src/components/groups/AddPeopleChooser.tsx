...
/**
 * One entry point for "add people to a team" that forks the two very different
 * paths so a captain can't confuse them:
 *   ① Invite to the team   → they create an account and become a member.
 *   ② Track availability    → account-less name; RSVPs by link, can join later.
 * Each card reveals its existing sub-panel in place.
 */
export default function AddPeopleChooser({
  ...
  const [mode, setMode] = useState<"choose" | "invite" | "availability">("choose");
      ...
      <div className="relative w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[85vh] overflow-y-auto p-5">
        ...
        {mode === "choose" && (
          <div className="space-y-3">
            <p className="text-xs text-gray-500">Two ways to add someone — pick what fits.</p>
            <button
